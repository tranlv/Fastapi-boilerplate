...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        user = g.current_user
        if user is None and user.admin is False:
            return send_error(message=i18n.t("authentication.error.not_login"), code=401)

        if user.is_deactivated:
            return send_error(message=i18n.t("authentication.error.user_deactivated"), code=401)
    ...
    def inner(f):
        ...
        def decorated(self, data, *args, **kwargs):
            if not isinstance(data, dict):
                return send_error(message=i18n.t("validation.error.wrong_data_format"))
